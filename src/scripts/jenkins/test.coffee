...
#   A test job is a regular Jenkins job, which has the scope to run test suite
# for a given project
#
# Commands:
#   hubot test <repo> on <environment> [from <branch>] - Test the application on
# a certain environment. If a branch is specified and the job is parametrized,
...
module.exports = (robot) ->
  ...
  robot.respond /test (.*) on (\w+) from ([\w-\/\#]+)$/i, (msg) ->
    ...
    # The Jenkins job has to be configured beforehand to be able to receive a
    # parameter, named "branch"
    appRepo = msg.match[1]
    appEnv = msg.match[2]
