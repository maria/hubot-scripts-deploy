...
#
# Commands:
#   hubot deploy <repo> on <environment> [from <branch>] - Deploy the application
# on a certain environment. If a branch is specified and the job is parametrized,
# then it will set the branch.
...
module.exports = (robot) ->
  ...
  robot.respond /deploy (.*) on (\w+) from ([\w-\/\#]+)$/i, (msg) ->
    # Trigger a job for a given repo and environment, specifying the "branch"
    # parameter. The Jenkins job has to be configured beforehand to be able to
    # receive a parameter, named "branch"
    appRepo = msg.match[1]
    appEnv = msg.match[2]
